...
class UpDownFold(OrigamiFrameTransition):
    """
    Base class for up/down vertical origami-style folds.
    ...
    # ---- Phase 1: fold top/bottom half of from_img forward ----
    ...
    def _phase1_vertex_shader(self):
        rotate_condition = "< 0.0" if self.direction == "up" else "> 0.0"
        rotate_sign = "-" if self.direction == "up" else ""
        return f"""
        #version 330
        in vec3 in_position;
        in vec2 in_texcoord;
        ...
        void main() {{
            ...
            uv = in_texcoord;
            if (pos.y {rotate_condition}) {{
                float angle = fold_progress * 1.57079;
                float d = abs(pos.y);
                pos.y = {rotate_sign}d * cos(angle);
                pos.z = d * sin(angle);
            }}
            pos.z += z_offset;
...
class OrigamiFoldUp(UpDownFold):
